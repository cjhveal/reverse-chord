...
piu.k_combinations = function k_combinations(set, k) {
...
piu.permutate = function permutate(set) {
...
possibleTriads = function(notes) {
  ...
  piu.k_combinations(n, 3).forEach(function(comb) {
    piu.permutate(comb).forEach(function(perm) {
      combs.push(perm);
    });
  });
...
piu.infer = function infer(notes, enharmonics) {
  var triads, chords = [];
  ...
  if (enharmonics) {
    ...
    chords = piu.k_combinations(notes, n).map(function(notecombi) {
